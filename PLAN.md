...
**다음 할 일:**
- [ ] 🔴 **검색 결과에서 내 구독이 반영되지 않는 버그** (2026-07-27 발견, 우선순위 최상)
      - 증상: 검색 카드가 구독 상태를 무시한다. 배지가 "바로 보기" 대신 "구독",
        결론이 "내 구독으로 무료 · wavve" 대신 **"대여 ₩1,300"**, 로고 강조·정렬도 안 됨.
        **이미 볼 수 있는 작품에 돈을 내라고 하는 것**이라 v0.4.0 원칙("틀린 숫자는
        숫자가 없는 것보다 나쁘다")에 정면으로 어긋난다.
      - 범위: **검색 페이지만.** 작품 상세(`/title/...`)는 정상 ("지금 바로 볼 수 있어요").
      - 결과 건수에 따라 갈린다: "인셉션"(3건) 정상, "스파이더맨"(20건) 재현.
      - 데이터·훅은 정상으로 확인됨:
        - `ResultsView` 안에서 찍어보면 `isLoaded: true`, `tmdbIds: [8,1796,1883,356]`
        - 제공처도 정상 (홈커밍 카드에 Netflix 로고가 실제로 그려짐)
        - `mergeAvailability()`는 합집합이라 flatrate 를 떨어뜨리지 않음
      - 즉 **데이터 문제가 아니라 카드 서브트리가 하이드레이션 후 다시 그려지지 않는
        문제**로 보인다. SSR 은 구독 정보를 모르므로(`getServerSnapshot` → `slugs: []`)
        "대여"로 렌더되고, 그 DOM 이 그대로 남는다.
      - 커밋된 코드에서 재현됨(작업 중 변경을 stash 하고 확인). 기존 버그다.
