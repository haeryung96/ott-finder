...
// (2) 실제 대여/구매 금액은 주지 않는다. 그래서 v0.2.0 까지는 서비스 검색 URL 과
// 표준 단가 '추정치'로 대신했는데, 실측해 보니 추정 오차가 컸다
// (인셉션 대여: 추정 ₩2,500 vs 실제 ₩1,320).
//
// ⚠️ 이 엔드포인트는 **비공식·미문서화**다 (introspection 도 막혀 있음).
// 스키마가 바뀌거나 차단되면 조용히 실패할 수 있으므로, 이 모듈은 **어떤 실패에서도
// 예외를 던지지 않고 null 을 반환**한다. 호출부는 null 이면 기존 동작
// (검색 URL + 표준 단가 추정치)으로 그대로 폴백한다 — JustWatch 가 죽어도 앱은 살아 있다.
//
